...
from Assignment5.GAME.Actions import ActionRegistry
...
class Agent:
    ...
    def run(self, user_input: str, memory=None, max_iterations: int = 50) -> Memory:
        ...
        for _ in range(max_iterations):
            # 1. Construct prompt that includes the Goals, Actions, and the current Memory
            prompt = self.construct_prompt(self.goals, memory, self.actions)

            print("Agent thinking...")
            # 2. Generate response from LLM agent
            response = self.prompt_llm_for_action(prompt)
            print(f"Agent Decision: {response}")

            # 3. Parse response to determine which action the agent wants to execute
